...
% demo_figs(t, c, v);
  ...
  % time [s], volume [m3], concentration [mmole/l=mole/m3]
	...
	V_in  = 1E-3;  % [m3]
        ...
        Km_A = 1.0;     % [mM]
        Vmax_b1 = 5.0;  % [mole/s]
        b1 = Vmax_b1/Km_A * (A_out - A_in)/(1 + A_out/Km_A + A_in/Km_A); % [mole/s]
        
        % b2 : B export
        Km_B = 0.5;     % [mM]
        Vmax_b2 = 2.0;  % [mole/s] 
        b2 = Vmax_b2/Km_B * (B_in - B_out)/(1 + B_out/Km_B + B_in/Km_B); % [mole/s]
        
        % b3 : C export
        Km_C = 3.0;     % [mM]
        Vmax_b3 = 2.0;  % [mole/s] 
        b3 = Vmax_b3/Km_C * (C_in - C_out)/(1 + C_out/Km_C + C_in/Km_C); % [mole/s]
        
        % v1 : C (A_in -> B_in)
        Keq_v1 = 10.0;  % [-]
        Vmax_v1 = 1.0;  % [mole/s]
        v1 = Vmax_v1/Km_A*(A_in - 1/Keq_v1*B_in); % [mole/s]
        
        % v2 : C (A_in -> C_in)
        Vmax_v2 = 0.5;  % [mole/s] 
        v2 = Vmax_v2/Km_A * A_in; % [mole/s] 
        
        % v3 : C (C_in -> A_in)
        Vmax_v3 = 0.1;  % [mole/s] 
        v3 = Vmax_v3/Km_A * C_in;  % [mole/s]
        
        % v4 : C (C_in -> B_in) 
        Keq_v4 = 2.0;   % [-]
        Vmax_v4 = 0.5;  % [mole/s]
        v4 = Vmax_v4/Km_A*(C_in - 1/Keq_v4*B_in); % [mole/s]
        
	% Changes in [mole/s/m3] = [mM/s]
        dydt = zeros(size(y));
        %A_in         = y(1);
        dydt(1) = (b1 -v1 -v2 +v3)/V_in;
        %B_in         = y(2);
        dydt(2) = (-b2 +v1 +v4)/V_in;
        %C_in         = y(3);
        dydt(3) = (-b3 +v2 -v3 -v4)/V_in;
        %A_out        = y(4);
        dydt(4) = (-b1)/V_out;
        %B_out        = y(5);
        dydt(5) = (+b2)/V_out;
