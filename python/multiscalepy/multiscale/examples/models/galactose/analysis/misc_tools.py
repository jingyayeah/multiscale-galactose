...
# ----------------------------------------------------------------------
# Averaging simulation time courses
# ----------------------------------------------------------------------
# The averaging is one of the crucial operations. The main challenge is
# the difference in time points between the different time courses.
# A first step is an interpolation so that all time courses are evaluated
# at the same time points.
def average_results(f_list, weights, ids, time, selections):
    """ Averaging of set of timecourses. with given weights. """

    # TODO: fix & refactor for general use
    # TODO: test
...
# Analyse peaks
# ----------------------------------------------------------------------
def find_peaks(s, compounds, maximum=True):
    """ Calculate the time of maximum and the maximal value for all given
        compounds. """
    # TODO: write test
    peaks = {}
    for key in compounds:
        ...
        if maximum:
            ...
            peak_value = min(data)
        peak_index = [i for i, item in enumerate(data) if item == peak_value]
        peak_time = times[peak_index[0]]
        peaks[key] = peak_time, peak_value
    return peaks

def print_peaks(peak_dict):
    """ Print the peak information. """
    # TODO: write test
    print('{:20s}{:10s}{:10s}'.format('sid', 'time', 'max'))
    for key in peak_dict:
        values = peak_dict[key]
        print('{:20s}{:5.3f}  {:5.3f}'.format(key, values[0], values[1]))

# ----------------------------------------------------------------------
# Flux & pressure distributions in the sinusoidal unit
# ----------------------------------------------------------------------

def flux_sample():
    """ Returns sampled fluxes for the integration. """
    flux = np.arange(start=10, stop=800, step=50)*1E-6  # m/s
    return np.sort(flux)
...
def pressure_sample(Pb=2.0):
    ...
    Pb = 2 [mmHg]
    """
    # TODO: proper handeling of the offset of 2 mmHg
