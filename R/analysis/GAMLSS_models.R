...
#######################################################
g <- gender.cols() # plot information for different genders
...
for (k in 1:3){
  ...
  points(d[inds.po, xname], d[inds.po, yname], col=g$cols.base[k], pch=g$symbols[k])
  inds.in <- which(d$dtype == 'individual')
  points(d[inds.in, xname], d[inds.in, yname], col=g$cols.base[k], bg=g$cols[k], pch=g$symbols[k])
