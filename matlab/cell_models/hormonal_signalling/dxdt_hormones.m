function [dydt, v] = dxdt_hormones(t, y)
% dxdt_hormones
% Sub-modul for the time dependent calculation of phosphorylation states.
% More complex implementation of the very simple instantanous dose response
% curves. 
%
% time [sec]
...
% idealized instantaneous dose response
ins_ref = max(0.0, (x_ins1-x_ins2) * glc_ext^x_ins4/(glc_ext^x_ins4 + x_ins3^x_ins4));

% exponential decay to expected dose response steady state 
tau_ins = 10; %[sec]
v_ins = [ins - ins_ref]*exp(-t/
...
x_glu4 = 6.40;
% idealized instantaneous dose response
glu_ref = max(0.0, (x_glu1-x_glu2)*(1 - glc_ext^x_glu4/(glc_ext^x_glu4 + x_glu3^x_glu4)));
...
x_epi4 = 8.40;
% idealized instantaneous dose response
epi_ref = max(0.0, (x_epi1-x_epi2) * (1 - glc_ext^x_epi4/(glc_ext^x_epi4 + x_epi3^x_glu4)));
...
dydt(2) = v_ins;     % +v2 +v10 +v11 +2*v12 +v13 +v15 -v20 -v23;   %adp
dydt(3) = v_glu;    % -v12;         %amp
dydt(4) = v_epi;     %utp
dydt(5) = v_gamma;
...
% steady state values which should be reached under the given
% hormonal concentrations
% These steady state concentrations are assymptotically reached
gamma_ss = 0.5 * (1 - ins/(ins+K_ins) + max(glu/(glu+K_glu), epi_f*epi/(epi+K_epi)) );
...
% Set additional concentrations constant
dydt(17) = 0; % glycogen
