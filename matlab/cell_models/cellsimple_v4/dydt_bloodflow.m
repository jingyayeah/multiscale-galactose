...
if p.ode_cells
    %disp('[1] CELLS')
    y_cell     = zeros(p.Nxc,1);
    dydt_cell = y_cell;
    for ci = 1:p.Nc
        y_cell(:) = y(p.Nx_out + (ci-1)*p.Nxc+1 : p.Nx_out + ci*p.Nxc);            
        dydt_cell(:) = dydt_cellsimple(t, y_cell, p);
        %dydt_cell(:) = dydt_f(t,y_cell,p);
        dydt(p.Nx_out + (ci-1)*p.Nxc+1 : p.Nx_out + ci*p.Nxc) = dydt_cell;
    end
end
...
if p.ext_constant
    % disp('no changes in adjacent compartments -> set constant')
    for ci=1:p.Nc
