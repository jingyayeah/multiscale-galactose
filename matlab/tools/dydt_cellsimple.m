...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  v_mct1_out = vmax(3)*infac/p.Nf * (lac_ext - lac_in/10.0)./ ...
            (onevec + lac_ext/6.0 + lac_in/6.0);
  v_mct1_in  = sum(v_mct1_out);
  
  % [4] v_mct4 (lac_ext -> lac) [export]
  v_mct4_out = vmax(4)*repfac/p.Nf * (lac_ext - lac_in/0.1)./ ...
            (onevec + lac_ext/30.0 + lac_in/30.0);
  v_mct4_in  = sum(v_mct4_out);
  
  % [5] v_ldha (lac -> pyr) [lac -> pyr]
  % more in the direction of pyruvate
  v_ldha = vmax(5)*repfac * (lac - pyr/10) / (lac + 6.88*(1 + pyr/0.5));
                        
  % [6] v_ldhb (lac -> pyr) [pyr -> lac]
  % more in the direction of lactate
  v_ldhb = vmax(6)*infac*(lac - pyr/0.1) / (lac + 3.95*(1 + pyr/0.5));
  ...
  v_anaerob = vmax(8)*infac* glc/(glc + 0.9) * adp/(adp + 0.3);
  %v_anaerob = max(0, v_anaerob);
  
  % [9] v_aerob (2 pyr + 6.3 o2 + 27.8 adp + 27.8 p -> 27.8 atp)
  v_aerob = vmax(9)*repfac * pyr/(pyr + 0.05) * ... 
                             adp/(adp + 0.3)  * ...
                             o2/ (o2 + 0.01);
  %v_aerob = max(0, v_aerob);
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % o2
  dydt(p.Nx_out*p.Nf + 3) = v_do2_in - 6.3*v_aerob;
  % atp  
  dydt(p.Nx_out*p.Nf + 4) =  2*v_anaerob + 27.8*v_aerob - v_atpase;
  % adp
  dydt(p.Nx_out*p.Nf + 5) = -2*v_anaerob - 27.8*v_aerob + v_atpase;
  % pyr
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargout > 1
    V_names{1} = 'v_anaerob';    V(1) = v_anaerob;
    V_names{2} = 'v_aerob';      V(2) = v_aerob;
    ...
    V_names{9} = 'v_do2_in';     V(9) = v_do2_in;    
end 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%